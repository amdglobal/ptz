...
int getPTRCurrent(uvc_device_handle_t* devicehandle, struct PanTiltRelative* value) {
    enum uvc_req_code requestCode = UVC_GET_CUR;
    uvc_error_t       res;
    res = uvc_get_pantilt_rel(devicehandle,
                              &value->panRelative,
                              &value->panSpeed,
                              &value->tiltRelative,
                              &value->tiltSpeed,
...
int setPTR(uvc_device_handle_t* devicehandle, struct PanTiltRelative value) {
    uvc_error_t res;
    res = uvc_set_pantilt_rel(
        devicehandle, value.panRelative, value.panSpeed, value.tiltRelative, value.tiltSpeed);
    if (res != 0) {
...
int main(int argc, char** argv) {
    ...
    // res = uvc_find_device(ctx, &device, 0x46d, 0x848, NULL); /* filter devices: vendor_id,
    // product_id, "serial_num" */ Sony SRG-120DU, 1356, 2943 Logitech CC3000e 1133, 2120
        ...
        if (strcmp(input, "r") == 0) {
            ...
            if (result != 0) {
                ...
                printf("current pan relative %d speed %d current tilt relative %d speed %d\n",
                       ptrCurrent.panRelative,
                       ptrCurrent.panSpeed,
                       ptrCurrent.tiltRelative,
                       ptrCurrent.tiltSpeed);
            }
